...
var datashift_audio_engine = {
    ...
    init: function(init_url){
        if (init_url === undefined) init_url = this.routes.init_url;
        //
        ...
        // sync settings
        if (init_url !== undefined) {
            self.state = 'sent init request';
            $.ajax(
                {
                    method: "POST",
                    url: init_url,
        ...
        this.visual.volume.on('input change', function(update) {
    ...
    //
    load: function(load_url, is_new) {
        if (load_url === undefined) load_url = this.routes.load_url;
        var self = this;

        if (load_url !==  undefined) {
            self.state = 'sent load request';
            $.ajax(
                {
                    method: "GET",
                    url: load_url,
                ...
                if(!data.hasOwnProperty('disable_playlist')) {
                    if (data.hasOwnProperty('playlist_partial')) {
                        ...
                        self.playlist.forEach(function(track, index) {
    ...
    render_wave_from_pure_data: function(url){
        if (url === undefined) url = this.routes.load_url;
    ...
    new_page: function(url, fromBeginning){
        if (fromBeginning === undefined) fromBeginning = true;
        $.get(url).done(function(pure_data){
            ...
            datashift_audio_engine.playlist.forEach(function(track, index) {
                var duration = '<div class="datashift-audio-duration">' + formatTime(track.duration) + '</div>';
                var full_name = '<div class="datashift-audio-full-name">' + track.author + " - " + track.name + '</div>';
